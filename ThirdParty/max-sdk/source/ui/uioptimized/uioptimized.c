...
void uioptimized_mousemove(t_uioptimized* x, t_object* patcherview, t_pt pt, long modifiers)
{
    ...
    for (i = 0; i < x->j_rectangle; i++) {
        if (pt.x >= x->j_rects[i].x * rect.width - x->j_rects[i].width * (rect.width * 0.05) && pt.x <= x->j_rects[i].x * rect.width + x->j_rects[i].width * (rect.width * 0.05) && pt.y >= x->j_rects[i].y * rect.height - x->j_rects[i].height * (rect.height * 0.05) && pt.y <= x->j_rects[i].y * rect.height + x->j_rects[i].height * (rect.height * 0.05)) {
