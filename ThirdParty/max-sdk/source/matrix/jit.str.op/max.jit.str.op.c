    ...
    if (argc && argv) {
        ...
        if (matrix && jit_object_method(matrix, _jit_sym_class_jit_matrix)) {
            if (inlet = max_jit_obex_inletnumber_get(x)) {
                // right inputs
                if ((p = jit_object_method(mop, _jit_sym_getinput, inlet + 1)) && (m = jit_object_method(p, _jit_sym_getmatrix)))
                {
                ...
                switch (jit_attr_getlong(mop, _jit_sym_outputmode)) {
                    ...
                    case 2: // input(no calc)
                        ...
                        if ((p = jit_object_method(mop, _jit_sym_getoutput, 1)) && (o = max_jit_mop_io_getoutlet(p)))
                        {
                    ...
                    default: // calc
                        ...
                        if ((p = jit_object_method(mop, _jit_sym_getoutput, 1)) && (ioproc = (method)jit_object_method(p, _jit_sym_getioproc)))
                        {
                            CALL_METHOD((*ioproc), mop, p, matrix);
                        }
