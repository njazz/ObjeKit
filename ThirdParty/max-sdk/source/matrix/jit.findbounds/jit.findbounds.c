    ...
    switch (dimcount) {
        ...
        default:
            for (i = 0; i < dim[dimcount - 1]; i++) {
                ...
                if (inrange0) {
                    inrange = TRUE;
                    if ((vecdata->v_char.boundmin[dimcount - 1] == -1) || (i < vecdata->v_char.boundmin[dimcount - 1])) {
                                       ...
                                       t_jit_matrix_info* in_minfo, char* bip)
{
    ...
    if (min0 < 0x7FFFFFFF) {
        vecdata->v_char.boundmin[0] = min0;
    }
    if (min1 < 0x7FFFFFFF) {
        vecdata->v_char.boundmin[1] = min1;
    }
    if (max0 > -1) {
        vecdata->v_char.boundmax[0] = max0;
    }
    else {
        vecdata->v_char.boundmax[0] = vecdata->v_char.boundmin[0];
    }
    if (max1 > -1) {
        vecdata->v_char.boundmax[1] = max1;
    }
    else {
        vecdata->v_char.boundmax[1] = vecdata->v_char.boundmin[1];
    }
                                ...
                                t_jit_matrix_info* in_minfo, char* bip)
{
    ...
    if (min0 < 0x7FFFFFFF) {
        vecdata->v_long.boundmin[0] = min0;
    }
    if (min1 < 0x7FFFFFFF) {
        vecdata->v_long.boundmin[1] = min1;
    }
    if (max0 > -1) {
        vecdata->v_long.boundmax[0] = max0;
    }
    else {
        vecdata->v_long.boundmax[0] = vecdata->v_long.boundmin[0];
    }
    if (max1 > -1) {
        vecdata->v_long.boundmax[1] = max1;
    }
    else {
        vecdata->v_long.boundmax[1] = vecdata->v_long.boundmin[1];
    }
                                   ...
                                   t_jit_matrix_info* in_minfo, char* bip)
{
    ...
    if (min0 < 0x7FFFFFFF) {
        vecdata->v_float32.boundmin[0] = min0;
    }
    if (min1 < 0x7FFFFFFF) {
        vecdata->v_float32.boundmin[1] = min1;
    }
    if (max0 > -1) {
        vecdata->v_float32.boundmax[0] = max0;
    }
    else {
        vecdata->v_float32.boundmax[0] = vecdata->v_float32.boundmin[0];
    }
    if (max1 > -1) {
        vecdata->v_float32.boundmax[1] = max1;
    }
    else {
        vecdata->v_float32.boundmax[1] = vecdata->v_float32.boundmin[1];
    }
                                   ...
                                   t_jit_matrix_info* in_minfo, char* bip)
{
    ...
    if (min0 < 0x7FFFFFFF) {
        vecdata->v_float64.boundmin[0] = min0;
    }
    if (min1 < 0x7FFFFFFF) {
        vecdata->v_float64.boundmin[1] = min1;
    }
    if (max0 > -1) {
        vecdata->v_float64.boundmax[0] = max0;
    }
    else {
        vecdata->v_float64.boundmax[0] = vecdata->v_float64.boundmin[0];
    }
    if (max1 > -1) {
        vecdata->v_float64.boundmax[1] = max1;
    }
    else {
        vecdata->v_float64.boundmax[1] = vecdata->v_float64.boundmin[1];
    }
